const NoiseShader = {
    ...
    uniforms: {
        ...
        speed: { type: 'v3', value: new THREE.Vector3(1, 0.5, 0.1) },
        scale: { type: 'v2', value: new THREE.Vector2(1, 1)  },
        offset: { type: 'v2', value: new THREE.Vector2(0, 0) },
        ease: { type: 'f', value: 1.0 },
    },
    ...
    fragment: [
        ...
        'vec4 noise( vec2 uv ) {',
        '    float n = 0.0;',
        '    vec3 p1 = vec3( v_uv, 1. ) + vec3( speed * time );',

        //# perlin noise
        //# '    n.xyz += 1.00 * cnoise( p * 1. );'
        //# '    n.xyz += 0.50 * cnoise( p * 2. );'

        //# simplex noise
        '    n += 1.00 * abs( snoise( p1 * 1. ) );',
        '    n += 0.80 * abs( snoise( p1 * 2. ) );',
        '    n += 0.50 * abs( snoise( p1 * 4. ) );',
        '    n = n * multiplier;',
        '    return vec4( color, 1. ) * vec4( n, n, n, 1. );',
        '}',

        'void main() {',
        '    gl_FragColor = noise( v_uv ) * ease;',
        '}',
